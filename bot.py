...
async def join_vc(interaction: discord.Interaction):
    ...
    try:
        ...
        # If getting "already connected", tell user to use /leave first
        await interaction.followup.send(
            "⚠️ Bot is already in a voice channel!\n"
            "Use `/leave` first, then try `/vc` again.",
            ephemeral=True
        )
        ...
        # Bot might have joined despite the error
        await interaction.followup.send(
            f"⚠️ Possible connection issue: {e}\n"
            f"Check if I'm in voice with `/status`"
        )
        # Try to register the connection anyway
        try:
            for vc in bot.voice_clients:
                if vc.guild.id == guild_id and vc.is_connected():
                    state.voice_clients[guild_id] = vc
                    break
        except:
            pass
