...
async def join_vc(interaction: discord.Interaction):
    ...
    try:
        # Force cleanup of any existing connection in our state
        if guild_id in state.voice_clients:
            try:
                old_client = state.voice_clients[guild_id]
        ...
        if voice_client.is_connected():
            await interaction.followup.send(
                ...
                f"Use `/texttospeech` to make me speak, or `/status` to check connection."
            )
        else:
            await interaction.followup.send(
                f"⚠️ Joined but connection is unstable. This is a known Railway limitation.\n"
                f"Try `/texttospeech Hello!` immediately to keep connection active."
            )
        
    except discord.ClientException:
        await interaction.followup.send(
            "⚠️ Bot thinks it's already connected!\n"
            "Use `/leave` first, wait 3 seconds, then try `/vc` again.",
            ephemeral=True
        )
            
    except asyncio.TimeoutError:
        await interaction.followup.send("❌ Connection timed out. Railway voice connections can be unstable.", ephemeral=True)
        
    except Exception as e:
        await interaction.followup.send(
            f"⚠️ Connection issue: {e}\n"
            f"Voice connections are limited on Railway. Use `/status` to check if I'm actually connected."
        )
